...
export function retentionView(doc,kind,meta={},operations={},now=Date.now()){
 ...
 return {retirementBlock:core?retirementBlock(doc,meta,operations,now):'Not a completed event.',id:kind==='mail'?doc.messageId:doc.id,recordVersion:doc.version||null,removalBlock:kind==='event'?disposalEligibilityForView(doc,meta,operations,now):mailDisposalEligibility(doc,meta,now),kind,version:meta.version||0,hold:meta.hold||false,holdReason:meta.holdReason||'',closedOn:meta.closedOn||'',taxDate:meta.taxDate||'',reviewedAt:meta.reviewedAt||null,reviewAt,reason,status:meta.hold?'On hold':!reviewAt?'Needs dates':reviewAt<=new Date(now).toISOString().slice(0,10)?'Review due':'Keep',core};
}
