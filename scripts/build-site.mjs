...
const flat=p=>Array.isArray(p.bands);
const packageFrom=p=>flat(p)?money(p.bands[0][1]):`$${p.rate}`;
const packageUnit=p=>flat(p)?'flat, per event':'/ guest';
const bandLine=p=>p.bands.map(([max,price],n)=>`${n?`to ${max}`:`Up to ${max} guests`}: ${money(price)}`).join(' &middot; ');
const CO=c.packages.collins;
const COLLINS_BODY=`<section class="split"><div><p class="price">${money(CO.bands[0][1])}<small> flat, per event</small></p><p>Two-hour baseline &middot; for rooms that already have a bar</p><p>Guests walk up and ask for a drink the way they would ask a bartender. Collins answers in its own voice, and it is held to the list you are actually pouring &mdash; name the drinks behind your bar and it recommends those. Ask it for something else and it steers you back.</p><p>Your team sets the list on the night. Photograph the bottles and Collins reads the shelf, you confirm what is there, and it locks to what that bar can genuinely make.</p></div><div><h2>Included, not extra</h2><ul>${CO.features.map(f=>`<li>${esc(f)}</li>`).join('')}${CO.serviceFeatures.map(f=>`<li>${esc(f)}</li>`).join('')}</ul><p>Extra hours, list changes after lock, events outside Manhattan and additional iPads are confirmed in your proposal.</p><a href="/#packages">Choose Collins in the planner &#8599;</a></div></section><section class="split"><div><h2>Bigger rooms, more stations</h2><p>One station serves roughly 25 to 30 guests comfortably. Past that the room queues at exactly the moment it is busiest, so the deployment grows rather than the rate per head.</p><ul>${CO.bands.map(([max,price],n)=>`<li>${n?`${c.packages.collins.bands[n-1][0]+1} to ${max}`:`Up to ${max} guests`} &mdash; ${money(price)}</li>`).join('')}<li>Above ${CO.bands[CO.bands.length-1][0]} &mdash; quoted with your proposal.</li></ul><p>Additional service time is ${money(250)} an hour up to 75 guests, ${money(375)} to 150 and ${money(500)} to 250 &mdash; an extra Collins hour is staff time, not a full bar.</p></div><div><h2>What it is not</h2><p>Collins does not pour. It recommends, explains and keeps a room moving &mdash; your bar, your bartenders and your spirits stay yours. There is no Barsys bar, no machines and no Barsys menus in this package; if you want those, that is Classic, Signature or Reserve.</p><p>Which is the point. Collins is for the office or venue that already has a bar it likes and wants the interactive layer on top of it.</p><a href="/#proposal">Talk it through &#8599;</a></div></section>`;
page('packages','A package for your kind of gathering.','Compare the Classic, Signature, Reserve and Collins happy-hour packages from Barsys: per-guest rates for a Barsys bar, a flat fee for Collins on your own bar, and what each package includes.',cards4(Object.entries(c.packages).map(([id,p])=>card(p.name,`<p class="price">${packageFrom(p)}<small> ${packageUnit(p)}</small></p><p>${esc(p.description)}</p>${flat(p)?`<p class="card-note">${bandLine(p)}. Your bar, your bartenders, your drinks list \u2014 we bring the AI bartender to it.</p>`:''}`,`/site/${id}.html`)))+'<p>Two-hour baseline. Tax and extras are confirmed in your proposal. Collins is priced per event rather than per guest.</p>','/assets/tech/360-hero-hd.jpg');
...
page('collins','Collins','Collins is the Barsys AI bartender for New York City rooms that already have a bar: a flat fee per event rather than a rate per guest, held to the drinks list you are already pouring, with iPads, stands, staff on site and your branding included.',COLLINS_BODY,'/assets/tech/360-hero-hd.jpg','A bartender your guests can talk to.');
