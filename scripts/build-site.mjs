...
const COLLINS_BODY=`<section class="split"><div><p class="price">${money(CO.bands[0][1])}<small> flat, per event</small></p><p>Two-hour baseline &middot; for rooms that already have a bar</p><p>Guests walk up and ask for a drink the way they would ask a bartender. Collins answers in its own voice, and it is held to the list you are actually pouring &mdash; name the drinks behind your bar and it recommends those. Ask it for something else and it steers you back.</p><p>Your team sets the list on the night. Photograph the bottles and Collins reads the shelf, you confirm what is there, and it locks to what that bar can genuinely make.</p></div><div><h2>Included, not extra</h2><ul>${CO.features.map(f=>`<li>${esc(f)}</li>`).join('')}${CO.serviceFeatures.map(f=>`<li>${esc(f)}</li>`).join('')}</ul><p>Extra hours, list changes after lock, events outside Manhattan and additional iPads are confirmed in your proposal.</p><a href="/#packages">Choose Collins in the planner &#8599;</a></div></section><section class="split"><div><h2>Bigger rooms, more stations</h2><p>One station serves roughly 25 to 30 guests comfortably. Past that the room queues at exactly the moment it is busiest, so the deployment grows rather than the rate per head.</p><ul>${CO.bands.map(([max,price],n)=>`<li>${n?`${c.packages.collins.bands[n-1][0]+1} to ${max}`:`Up to ${max} guests`} &mdash; ${money(price)}</li>`).join('')}<li>Above ${CO.bands[CO.bands.length-1][0]} &mdash; quoted with your proposal.</li></ul><p>Additional service time is ${money(CH.price)} an hour at any size &mdash; an extra Collins hour is our team running the stations, not a full bar.</p></div><div><h2>What it is not</h2><p>Collins does not pour. It recommends, explains and keeps a room moving &mdash; your bar, your bartenders and your spirits stay yours. There is no Barsys bar, no machines and no Barsys menus in this package; if you want those, that is Classic, Signature or Reserve.</p><p>Which is the point. Collins is for the office or venue that already has a bar it likes and wants the interactive layer on top of it.</p><a href="/#proposal">Talk it through &#8599;</a></div></section>`;
