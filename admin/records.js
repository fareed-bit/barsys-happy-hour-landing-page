...
async function api(path,body,method='PATCH'){const r=await fetch('/api/admin/'+path,{method:body?method:'GET',headers:body?{'Content-Type':'application/json'}:{},...(body?{body:JSON.stringify(body)}:{})});const d=await r.json();if(r.status===401){location.replace('/admin/login.html?next='+encodeURIComponent(location.pathname+location.search));throw Error(d.error||'Sign in to continue.');}if(!r.ok)throw Error(d.error||'Request failed');return d;}
