...
(() => {
  ...
  function renderQuick(focus=false) {
    ...
    $('#quick-step-label').textContent=['The essentials','Review & your details','Your local preview'][quickStep];
    ...
    $('#quick-next').innerHTML=quickStep===0?`Next: your details ${icon('arrow')}`:`Create proposal preview ${icon('check')}`;
    if(quickStep===0) {
      ...
      $('#quick-content').innerHTML=`<h3 tabindex="-1">Where should we send the proposal?</h3><p class="step-description">Just your contact details. You can review the event summary below if you want to double-check anything.</p><div class="quick-fields quick-contact-fields">
      ...
      <details class="quick-review"><summary>Review my event <span>Optional check ${icon('chevron')}</span></summary>${A.reviewMarkup()}</details>${window.BarsysReadiness.serviceMarkup(s)}${window.BarsysReadiness.finalNotice(s,'quick')}<p class="review-info">Submitting a request does not hold a date or take payment. Final availability and scope are confirmed by Barsys.</p>`;
    } else {
      $('#quick-content').innerHTML=`<div class="quick-success"><div class="success-icon">${icon('check')}</div><h3 tabindex="-1">A very good idea.<br/>All in one place.</h3><p class="success-message">Your proposal summary is ready. <strong>Nothing has been sent to Barsys or booked yet.</strong>${window.BARSYS_RUNTIME?' Review it, then send your inquiry below.':''}</p><div class="quick-complete-note">The summary includes your menu choices, each add-on request, the priced subtotal and everything awaiting a quote.</div>${window.BarsysReadiness.timeline()}<div class="success-actions"><button type="button" class="button" data-open-summary>View event summary ${icon('arrow')}</button><button type="button" class="button button-outline" data-download-summary>Download summary ${icon('arrow')}</button><button type="button" class="text-link" data-copy-summary>Copy the details</button></div><button type="button" class="text-link" id="quick-edit">Edit the essentials ${icon('arrow')}</button></div>`;
    }
    updateQuickSummary();
  ...
  function approvalMarkup() {
    ...
    return `<div class="approval-header"><img class="approval-logo" alt="Barsys" width="170" height="32" src="${esc(C.brand.summaryLogoDataURI)}"/><div class="approval-version">EVENT PLANNING PREVIEW<br/>${esc(new Date().toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}))}</div></div>
    <h2 id="summary-title">Your next good time.<br/>Every detail, together.</h2><p class="approval-subtitle">${esc(c.company||c.name||'Your event')} / ${esc(e.occasion)}</p><span class="summary-safe">LOCAL PREVIEW ONLY - NOT SENT, BOOKED OR CHARGED</span>
    ...
    ${window.BarsysReadiness.summaryMarkup(d)}<div class="approval-next"><h3 style="margin-top:0">Next: confirm with the event team</h3><p>Contact ${esc(C.email)} to confirm availability, final scope, ingredient requirements, all unpriced requests, tax and program terms. Generating this summary sends nothing and does not reserve a date.</p></div><div class="approval-footer">A locally generated planning document, not an invoice or booking confirmation. Contact details appear because the organizer chose to export this plan.</div>`;
  }
