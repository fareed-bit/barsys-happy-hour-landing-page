...
test('V3.7: eight full films, with all four NYC uploads represented',()=>{
 assert.equal(Object.keys(M).length,23);
...
test('seventeen hero scenes (two archive films, six Park Terrace clips, nine 2026 Barsys happy hours), four experience previews and six photo cards',()=>{
 assert.equal((html.match(/data-scene="\d+"/g)||[]).length,17);
 assert.equal((html.match(/data-go-scene="\d+"/g)||[]).length,17);
 assert.ok(html.includes('<span>/ 17</span>'));
 assert.equal((html.match(/class="scene-inset"/g)||[]).length,17,'every scene carries a framed still');
 for(const k of ['rooftopPour','glassIn','shakenByHand','theRoom','windowPour','garnish','rooftopSunset','galentines','y2kNight','stPatricks','apresSki','springBar','pradaNight','mastersMartini','afterDark'])assert.ok(html.includes(`data-carousel-video="${k}"`)&&M[k]?.previewSrc,k);
