...
test('indexing fails closed in local and staging; sitemap excludes private routes',()=>{
 ...
 const s=seoSettings({origin:'https://example.com',indexing:'1',local:false,staging:false});assert.ok(sitemap(s).includes('https://example.com/site/classic.html'));assert.ok(!sitemap(s).includes('/admin'));assert.equal(sitePaths.size,49);
});
