...
**Status (2026-09-14 evening ET): candidate built; deployed at 0% traffic with `BARSYS_RECEIPT_BUCKET` set. Flip pending owner.** Brief: `receipt-upload-brief.md`. Rollback stays `match-0914`; receipts are additive, so rolling back only hides the upload form (uploaded objects stay in the bucket and the rows stay on the event).
