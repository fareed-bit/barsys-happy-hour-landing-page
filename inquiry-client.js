...
(()=>{'use strict';
 ...
 function attach(){for(const host of document.querySelectorAll('.success-actions')){if(host.parentElement.querySelector('[data-inquiry-save]'))continue;const section=document.createElement('section');section.dataset.inquirySave='';section.className='inquiry-submit-panel';section.style.cssText='border:1px solid #aaa;padding:20px;margin:24px 0';const heading=document.createElement('h4');heading.textContent=settings.mode==='LIVE'?'Ready for the Barsys team?':'Private preview submission';const p=document.createElement('p');p.textContent=settings.mode==='STAGING_TEST'?'Save this synthetic plan to the private staging database so the submission flow can be reviewed. Nothing is sent to the event team.':settings.mode==='LOCAL_TEST'?'Save this synthetic plan to the local event database. It will be visible in the local dashboard, but nothing is sent to the event team.':'Send this plan and contact details to the Barsys event team for review. This is an inquiry, not a reservation.';const b=document.createElement('button');b.type='button';b.className='button';b.textContent=settings.mode==='LIVE'?'Request a proposal':'Save test inquiry';const output=document.createElement('p');output.setAttribute('role','status');output.style.overflowWrap='anywhere';b.addEventListener('click',()=>save(b,output));section.append(heading,p,b,output);host.before(section);}}
