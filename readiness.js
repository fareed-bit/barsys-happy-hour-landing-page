...
(() => {
 ...
 function finalNotice(s,prefix='full'){
   return `<section class="request-clarity" aria-label="Before creating your preview"><p class="eyebrow">BEFORE YOU CONTINUE</p><h4>A plan, not a reservation.</h4><p>This action creates a local preview. It does not send an inquiry, sign an agreement, hold a date or collect a deposit. A booking requires a signed event agreement and the required deposit, then written confirmation.</p><p>Final pricing, availability, cancellation terms and deadlines are set in your event agreement. ${policyLink('terms','Event terms')} &middot; ${policyLink('cancellations','Cancellations & rescheduling')}</p>
   <div class="permission-options"><label class="permission-check"><input type="checkbox" data-memory-choice="marketingInterest" ${s.details.marketingInterest==='yes'?'checked':''}/><span>I would like information about future Barsys events and offers. <small>Optional. In this preview this records a preference only; it does not subscribe you.</small></span></label>
   ${select(s,prefix==='full'?'detail':'quick','photoPreference','Event publicity preference',maps.photoPreference,'This is not a photo release. Identifiable guest images require a separate approval process; no publication permission is granted by this form.')}</div>
   <p class="privacy-inline-notice">Your details stay in this tab unless you copy, print or download them. Marketing preferences and photo choices are separate from service agreement acceptance. ${policyLink('privacy','Privacy notice')}</p></section>`;
 }
 ...
 function summaryMarkup(data){const c=data.contact||{},r=data.readiness;return `<h3>Before this becomes a booking</h3><dl class="approval-grid">${[['Venue permission',label('venueApproval',c.venueApproval)],['Insurance paperwork',label('coi',c.coi)],['Glassware',label('glassware',c.glassware)],['Contracting entity',c.contractEntity||'To be confirmed at agreement stage'],['Marketing preference',c.marketingInterest==='yes'?'Interested / not subscribed':'No opt-in recorded'],['Publicity preference',label('photoPreference',c.photoPreference)+' / not publication permission']].map(([k,v])=>`<div class="approval-item"><dt>${k}</dt><dd>${esc(v)}</dd></div>`).join('')}</dl><p>Service is subject to responsible alcohol-service checks where applicable. Drinks are prepared in a shared environment; the absence of allergens cannot be guaranteed. Venue approval and a COI request do not themselves establish alcohol-service authorization or insurance coverage.</p>${timeline()}<p>Cancellation and rescheduling terms, deposit amount, sign-off dates and any overtime rate remain event-specific. This summary accepts no agreement and grants no publicity rights.</p>`;}
 window.BarsysReadiness=Object.freeze({eventFields,glasswareMarkup,serviceMarkup,finalNotice,timeline,summaryRows,exportMeta,summaryMarkup,label,policyLink});
